#!/usr/bin/env python3
"""Capture direct-ADC triggered_acquisitions and reconstruct a truthful gap-aware time axis."""
...
def main() -> int:
    p = argparse.ArgumentParser(description=(
        "Capture repeated 4000-sample triggered_acquisitions and place them on an irregular time axis "
    ...
    p.add_argument("--validate-square-hz", type=float, default=None,
                   help="validation only; report within-triggered square-wave frequency")
    ...
    csv_path = args.output_dir / f"{stamp}_{args.tag}_{args.triggered_acquisitions}triggered_acquisitions.csv"
    json_path = args.output_dir / f"{stamp}_{args.tag}_{args.triggered_acquisitions}triggered_acquisitions.json"

    print(f"Target: CH{cfg.channel}, A2={cfg.range_id:02X}, A3={cfg.a3:02X}, {cfg.sample_rate/1e6:.3f} MS/s")
    print(f"Triggered acquisitions: {args.triggered_acquisitions}; gap derived from successive measured A4 acquisition starts; arm delay={args.arm_delay_ms:g} ms")
    print("No samples are synthesized in missing time; CSV uses a NaN break between triggered_acquisitions.\n")

    triggered_acquisitions: list[list[int]] = []
    triggered_start_s: list[float] = []
    ...
    with Hantek1008C() as scope:
        ...
        for n in range(1, args.triggered_acquisitions + 1):
            ...
            if len(words) != 4000:
                print(f"triggered {n}: FAIL expected 4000 words, got {len(words)}")
                return 2
            cycle_s = (ended - started) / 1e9
            triggered_acquisitions.append(words)
            triggered_start_s.append(metrics["a4_start_ns"] / 1e9)
            call_elapsed_s.append(cycle_s)
            sample_ms = len(words) / cfg.sample_rate * 1000.0
            freq_text = ""
            if args.validate_square_hz is not None:
                freq, edges = estimate_square_frequency_per_triggered_acquisition(words, cfg.sample_rate)
                freq_text = f" freq={freq:.2f} Hz/{edges} rises" if freq is not None else f" freq=unresolved/{edges} rises"
            print(f"triggered {n:3d}: call={cycle_s*1000:.3f} ms real={sample_ms:.3f} ms A5={state}/{polls}{freq_text}")

    times, values, rows = build_gap_timeline(triggered_acquisitions, triggered_start_s, cfg.sample_rate)
    for row in rows[:-1]:
        print(f"gap after triggered {row.triggered:3d}: start-to-start={row.cycle_elapsed_s*1000:.3f} ms gap={row.gap_after_s*1000:.3f} ms")
    ...
    validation_rows = []
    if args.validate_square_hz is not None:
        for i, words in enumerate(triggered_acquisitions, 1):
            freq, edges = estimate_square_frequency_per_triggered_acquisition(words, cfg.sample_rate)
            validation_rows.append({"triggered": i, "frequency_hz": freq, "rising_edges": edges})
            if freq is not None:
    ...
    meta = {
        ...
        "triggered_samples": 4000,
        "triggered_count": len(rows),
        "gap_rule": "gap_after[i] = max(0, A4_start[i+1] - A4_start[i] - real_sample_duration); no inferred gap after final triggered",
        "a4_start_monotonic_s": triggered_start_s,
    ...
    print(f"  real triggered duration : {4000/cfg.sample_rate*1000:.3f} ms")
    if gaps_ms:
        print(f"  gap median          : {statistics.median(gaps_ms):.3f} ms")
        print(f"  gap min..max        : {min(gaps_ms):.3f} .. {max(gaps_ms):.3f} ms")
    else:
        print("  gap                 : unresolved (need >=2 triggered_acquisitions)")
    if frequencies:
        print(f"  within-triggered freq   : median={statistics.median(frequencies):.3f} Hz min={min(frequencies):.3f} max={max(frequencies):.3f}")
