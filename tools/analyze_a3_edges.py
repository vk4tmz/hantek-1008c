...
def parse_args():
    ...
    p.add_argument("--plot-dir", type=Path, default=None,
                   help="optional directory for raw waveform PNGs with detected major edges")
    return p.parse_args()
...
def main():
    ...
    for meta_path in args.captures:
        ...
        rate = f"{result.sample_rate:,.0f}" if result.sample_rate is not None else "INSUFFICIENT"
        print(
